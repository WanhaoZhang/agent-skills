...
MERMAID_NAME_PATTERN = re.compile(r"^codex-gpt5\.5-\d{8}-.+_mermaid\.md$")
TOUR_NAME_PATTERN = re.compile(r"^codex-gpt5\.5-\d{8}-.+_codetour\.tour$")
...
def check_mermaid_file(path: Path) -> list[str]:
    ...
    if not MERMAID_NAME_PATTERN.match(path.name):
        errors.append(
            f"{path}: Mermaid filename should look like "
            "codex-gpt5.5-YYYYMMDD-<feature>_mermaid.md."
        )
    ...
    if "Generated by:" not in text:
        errors.append(f"{path}: missing generated-by line.")
...
def check_tour_file(path: Path) -> list[str]:
    errors: list[str] = []
    if not TOUR_NAME_PATTERN.match(path.name):
        errors.append(
            f"{path}: CodeTour filename should look like "
            "codex-gpt5.5-YYYYMMDD-<feature>_codetour.tour."
        )
    try:
